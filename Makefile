...
PROTOCOL = $(patsubst %.cpp,%.o,$(wildcard Protocol/*.cpp))
...
##################################################################### EAA-Protocol Start###############################################
EAA_LDLIBS := $(OPENSSL_LDFLAGS) -lcrypto $(LDLIBS)

## our compilation
all: EAA

EAA: EAA-offline EAA-online

EAA-offline: pairwise-offline.x
EAA-online: CP.x DP.x Server.x

CP.x: CP.cpp $(COMMON) $(PROCESSOR) $(PROTOCOL)
	$(CXX) $(CFLAGS) CP.cpp -o CP.x $(COMMON) $(PROCESSOR) $(PROTOCOL) $(EAA_LDLIBS)

DP.x: DP.cpp $(COMMON) $(PROCESSOR) $(PROTOCOL)
	$(CXX) $(CFLAGS) DP.cpp -o DP.x $(COMMON) $(PROCESSOR) $(PROTOCOL) $(EAA_LDLIBS)

##################################################################### EAA-Protocol End###############################################


##################################################################### ESA-Protocol Start###############################################
ESA: esa-server.x esa-dp.x esa-cp.x acc-test

esa-dp.x: elgamal_benchmark.x ESA/DP.cpp $(COMMON) $(PROCESSOR)
	$(CXX) $(CFLAGS) -g -I ./ ESA/DP.cpp ElGamal_openssl.o -L /usr/local/lib -L /usr/local/lib64 -o ESA/DP.x $(COMMON) $(PROCESSOR) ./ESA/DataParty.cpp $(EAA_LDLIBS)

esa-cp.x: elgamal_benchmark.x ESA/CP.cpp $(COMMON) $(PROCESSOR)
	$(CXX) $(CFLAGS) -g -I ./ ESA/CP.cpp ElGamal_openssl.o -L /usr/local/lib -L /usr/local/lib64 -o ESA/CP.x $(COMMON) $(PROCESSOR) ./ESA/ComputationParty.cpp $(EAA_LDLIBS)

elgamal: ESA/ElGamal_openssl.cpp 
	$(CXX) $(OPENSSL_CPPFLAGS) -g -m64 -O2 -I ./ -c -Wno-deprecated-declarations -L /usr/local/lib -L /usr/local/lib64 ./ESA/ElGamal_openssl.cpp $(EAA_LDLIBS)

elgamal_benchmark.x: ESA/benchmark.cpp elgamal $(COMMON)
	$(CXX) $(OPENSSL_CPPFLAGS) -g -m64 -O2 -I ./ -Wno-deprecated-declarations -L /usr/local/lib -L /usr/local/lib64 ./ESA/benchmark.cpp ElGamal_openssl.o $(COMMON) -o ./ESA/benchmark.x $(EAA_LDLIBS)

esa-server.x: Server.cpp $(COMMON)
	$(CXX) $(CFLAGS) Server.cpp -L /usr/local/lib -L /usr/local/lib64 -o ./ESA/server.x $(COMMON) $(LDLIBS)

acc-test: acc-test-euro.x acc-test-crypto.x

acc-test-euro.x: DP/Euro_real.cpp 
	$(CXX) $(CFLAGS)  DP/Euro_real.cpp -o ./DP/Euro_real.x

acc-test-crypto.x: DP/crypto_sum.cpp 
	$(CXX) $(CFLAGS)  DP/crypto_sum.cpp -o ./DP/crypto_sum.x
