...
void DP::start()
{
  ...
  double epsilon = 1;
  int k = oM;
  double alpha = 1 / (double)n;//gamma distribution alpha
  double beta = 1 / epsilon;//gamma distribution beta
  std::gamma_distribution<double> distribution(alpha, beta);
  std::default_random_engine generator(time(0));
  std::vector<std::vector<double>> user_upload;
  std::vector<int>user_data;
  user_upload.resize(k);
  double gamma1, gamma2;
  int temp;
  srand((unsigned)time(NULL));
  for (int i = 0; i < n; i++)
        user_data.push_back(rand() % k); // geneate(int)data,  (0-1 float--rand() / double(RAND_MAX) )
  for (int i = 0; i < k; i++)
    for (int j = 0; j < n; j++)
    {
      gamma1 = distribution(generator);
      gamma2 = distribution(generator);
      if (user_data[j] == i)
      {
        temp = (1 + gamma1 - gamma2) * 10000;
        user_upload[i].push_back(temp);
      }
      else 
      { 
        temp = (gamma1 - gamma2) * 10000;
        user_upload[i].push_back(temp); 
      }
    }	
  ...
  tmp_gfp_array.resize(oM);
  for(int m=0; m<oM; m++){
    tmp_gfp_array[m].resize(uN);
    for(int i=0; i<uN; i++){
      temp=user_upload[m][i];
      tmp_gfp_array[m][i].assign(temp);
    }
  }

  //gettimeofday(&t1, NULL);
  if(nthreads>1){
    multi_thread_bucket_share_value(tmp_gfp_array);
  }else{
    bucket_share_value(tmp_gfp_array);
  }
  gettimeofday(&t2, NULL);
  cpu_time_used = (double)(t2.tv_sec-t1.tv_sec)*1000+(double)(t2.tv_usec-t1.tv_usec)/1000;
  printf("Encoder uploading procedure: %d clients each with %d messages need  %f (ms) \n",uN,oM,cpu_time_used);
...
bool DP::bucket_share_value(std::vector<std::vector<T>>& des_vec){
  if(!connection_active){
        throw runtime_error("The socket has expired");
  }
...
bool DP::share_value(T& x){
  ...
  cout<<"successfully complete share procedure\n";
  //
  return true;
}
